  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        // Update user profile display metadata
        await updateProfile(credential.user, {
          displayName: name.trim(),
          photoURL: selectedAvatar
        });

        // Sync to Firestore Users table
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {isSignUp && (
            <>
              {/* Nickname Input */}
              <div>
                <label className="block text-[10px] font-mono text-zinc-450 dark:text-zinc-400 tracking-wide uppercase mb-1">
                  Full Name
                </label>
              ...
              {/* Avatar selection */}
              <div>
                <label className="block text-[10px] font-mono text-zinc-450 dark:text-zinc-455 uppercase tracking-wide mb-1.5">
